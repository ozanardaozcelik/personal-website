...
OUT_FINAL = r'public/landing-pages/meng-to-sketchbook/lau-pa-sat.png'
ILLUS_PATH = r'C:\Users\dloozi\.gemini\antigravity-cli\brain\af0d76d1-4e98-4245-9246-8ea4316a204d\plate05_sahi_uav_1789635730454.jpg'
...
# 1. Kicker & Plate Number (Now LEVHA 05)
draw.text((LX, LY), 'BÖLÜM 05 // TEKNOFEST SAVAŞAN İHA · YAZILIM LİDERİ', font=font_kicker, fill=EARTH)
draw.text((LX + MAX_W - 70, LY), 'LEVHA 05', font=font_kicker, fill=INK_FAINT)
draw.line([(LX, LY + 22), (LX + MAX_W, LY + 22)], fill=RULE_COLOR, width=1)

# 2. Title & Subtitle
draw.text((LX, LY + 32), 'Şahi SİHA: Muharebe Otonomisi & Simülasyon', font=font_title, fill=INK_DEEP)
draw.text((LX, LY + 68), 'Şahi Otonom SİHA Takımı — Yazılım Ekip Liderliği', font=font_sub, fill=EARTH)
...
# Lead: Prior computer vision interest and joining as Software Team Lead
lead_text = 'Görüntü işlemeye duyduğum derin ilgi ve geliştirdiğim projelerimin ardından, Şahi Otonom SİHA Takımına Yazılım Ekip Lideri olarak katıldım.'
curr_y = LY + 102
...
# Body: YOLOv8, ArduPilot, ROS 2, Gazebo Harmonic, Kamikaze dive guidance
body_text = (
    'TEKNOFEST Savaşan İHA yarışması kapsamında; yüksek hızlı hava-hava muharebe senaryolarında '
    'gerçek zamanlı YOLOv8 nesne tespiti modellerini sahaya entegre ettim. ArduPilot, ROS 2 ve '
    'Gazebo Harmonic kullanarak telemetri haberleşmesini ve simülasyon test hattını kurdum. '
    'Milisaniyeler mertebesinde kilitlenen kamikaze dalış güdüm mimarilerini ve otonom durum '
    'makinelerini uçtan uca yönettim.'
)
...
curr_y += 16
note_text = '✦ "Simülasyondan gökyüzüne: ArduPilot, ROS 2, Gazebo Harmonic ve gerçek zamanlı YOLOv8 ile hava otonomisi."'
draw.text((LX, curr_y), note_text, font=font_note, fill=INK_FAINT)
curr_y += 20

signature_text = '[Şahi Otonom SİHA Yazılım Liderliği — YOLOv8, ArduPilot, ROS 2, Gazebo & Güdüm]'
draw.text((LX, curr_y), signature_text, font=font_signature, fill=INK_FAINT)

# 3. Right Page: Plate 05 Şahi SİHA Delta Wing, Dogfight & Guidance vectors
...
res.save(OUT_FINAL)
print(f'SUCCESS! Spread 05 (lau-pa-sat.png) saved to {OUT_FINAL} ({os.path.getsize(OUT_FINAL)} bytes)')
