...
<body>
  ...
  <header class="story-hero-section" id="story-section">
    ...
    <a href="#skills-section" class="story-scroll-cue" id="cueScrollToShelf" title="Raftaki Yerine Süzül">
      <span><span class="lang-tr">02 Yeteneklerim (Raftaki Yerine Süzül) ↓</span><span class="lang-en">02 Capabilities (Dock to Shelf) ↓</span></span>
  ...
  <!-- =========================================================================
       DIRECT DOCK TRANSITION OVERLAY (Zero intermediate page!)
       Directly folds cover shut, zooms out in 3D perspective, and docks into shelf
       ========================================================================= -->
  <div class="direct-dock-overlay" id="directDockOverlay" aria-hidden="true">
    <div class="direct-dock-stage">
      <div class="direct-dock-book" id="directDockBook">
        <!-- Spine -->
        <div class="dd-spine">
        ...
        <div class="dd-wing dd-left" id="ddWingLeft">
          <div class="dd-face dd-paper">
            <div class="dd-page-art">
              <span class="dd-tag">LEVHA 01 // 1999</span>
              <div class="dd-sketch-lead">3 YAŞ: İLK BİLGİSAYAR</div>
              <div class="dd-sketch-sub">Retro Oyunlar · CRT Monitör · İlk Kıvılcım</div>
            </div>
            <div style="font-family:var(--font-mono);font-size:9px;color:#9a6a3e;letter-spacing:0.12em;">TEORİK BİLGİ ➔ PRATİK</div>
          </div>
          <div class="dd-face dd-back-cover">
            <div class="dd-cover-foil">
              <div class="dd-roman">✦</div>
              <div>
                <div class="dd-title">OZAN ARDA</div>
                <div class="dd-sub">MAKİNE MANTIĞI</div>
              </div>
              <div class="dd-meta">2026 EDİSYON</div>
            </div>
          </div>
        </div>

        <!-- Right Wing (Front cover that folds shut over the book) -->
        <div class="dd-wing dd-right" id="ddWingRight">
          <div class="dd-face dd-paper">
            <div class="dd-page-art">
              <span class="dd-tag">LEVHA 09 // 2026</span>
              <div class="dd-sketch-lead">İNSAN FORMU — MAKİNE MANTIĞI</div>
              <div class="dd-sketch-sub">Şahi SİHA · Çözüm Makina · Gelecek Vizyonu</div>
            </div>
            <div style="font-family:var(--font-mono);font-size:9px;color:#9a6a3e;letter-spacing:0.12em;">SİHA YAZILIM LİDERİ</div>
          </div>
          <div class="dd-face dd-front-cover">
            <div class="dd-cover-foil">
              <div class="dd-roman">0</div>
              <div>
                <div class="dd-title">HİKAYEM</div>
                <div class="dd-sub">OZAN ARDA ÖZÇELİK</div>
              </div>
              <div class="dd-rule"></div>
              <div class="dd-meta">VOL. 00 // 9 DÖNÜM NOKTASI</div>
            </div>
          </div>
        </div>
      </div>
  ...
  <footer class="site-footer">
    ...
    <div class="footer-middle-grid">
      ...
      <div>
        ...
        <ul class="footer-links">
          <li><span>VOL. 2026 // BUILD 4.3</span></li>
  ...
  <script>
    (function () {
      ...
      // 2. Direct Book Closing, Zoom-Out & Shelf Docking Transition
      let isTransitioning = false;
      const directOverlay = document.getElementById('directDockOverlay');
      const ddBook = document.getElementById('directDockBook');
      const ddRight = document.getElementById('ddWingRight');
      ...
      function playDirectDockTransition(onComplete) {
        if (isTransitioning) return;
        isTransitioning = true;

        if (directOverlay && ddBook && ddRight) {
          // 1. Activate overlay directly over the open sketchbook
          directOverlay.style.opacity = '1';
          directOverlay.classList.add('is-active');
          directOverlay.classList.remove('bg-dark');
          ddRight.style.transform = 'rotateY(0deg)';
          ddBook.style.transform = 'translateY(0) scale(1) rotateY(0) rotateX(0)';

          // 2. Cover closes (folds shut in 3D)
          setTimeout(() => {
            ddRight.style.transform = 'rotateY(-180deg)';
          }, 40);

          // 3. Zoom out in 3D perspective and glide towards shelf coordinates
          setTimeout(() => {
            directOverlay.classList.add('bg-dark');
            ddBook.style.transform = 'translateY(16vh) scale(0.24) rotateY(18deg) rotateX(10deg)';
          }, 380);

          // 4. Settle in place and reveal skills shelf
          setTimeout(() => {
            if (skillsSec) {
              skillsSec.scrollIntoView({ behavior: 'auto' });
            }
            directOverlay.style.opacity = '0';
            setTimeout(() => {
              directOverlay.classList.remove('is-active', 'bg-dark');
              directOverlay.style.opacity = '';
              ddRight.style.transform = '';
              ddBook.style.transform = '';
      ...
      function playReverseDockTransition(onComplete) {
        if (isTransitioning) return;
        isTransitioning = true;

        if (directOverlay && ddBook && ddRight) {
          directOverlay.style.opacity = '1';
          directOverlay.classList.add('is-active', 'bg-dark');
          ddRight.style.transform = 'rotateY(-180deg)';
          ddBook.style.transform = 'translateY(16vh) scale(0.24) rotateY(18deg) rotateX(10deg)';

          // 1. Zoom in towards camera
          setTimeout(() => {
            directOverlay.classList.remove('bg-dark');
            ddBook.style.transform = 'translateY(0) scale(1) rotateY(0) rotateX(0)';
          }, 50);
          ...
          // 3. Reveal story sketchbook
          setTimeout(() => {
            if (storySec) {
              storySec.scrollIntoView({ behavior: 'auto' });
            }
            directOverlay.style.opacity = '0';
            setTimeout(() => {
              directOverlay.classList.remove('is-active');
              directOverlay.style.opacity = '';
              ddRight.style.transform = '';
              ddBook.style.transform = '';
              isTransitioning = false;
      ...
      // 3. Navigation Switcher Scroll-Spy & Interactive Transitions
      ...
      if (btnNavSkills) {
        btnNavSkills.addEventListener('click', (e) => {
          e.preventDefault();
          const skillsTop = skillsSec ? skillsSec.offsetTop - 100 : 800;
          if (window.scrollY < skillsTop) {
            playDirectDockTransition();
          } else {
            if (skillsSec) skillsSec.scrollIntoView({ behavior: 'smooth' });
          }
        });
      ...
      const cueScrollToShelf = document.getElementById('cueScrollToShelf');
      if (cueScrollToShelf) {
        cueScrollToShelf.addEventListener('click', (e) => {
          e.preventDefault();
          playDirectDockTransition();
        });
      }

      // Wheel scroll detection on 01 Hikayem to trigger direct dock transition
      let wheelDebounce = false;
      window.addEventListener('wheel', (e) => {
        if (isTransitioning || wheelDebounce) return;
        const scrollY = window.scrollY;

        // At top of page in story section and scrolling down
        if (scrollY < 60 && e.deltaY > 35) {
          wheelDebounce = true;
          setTimeout(() => { wheelDebounce = false; }, 1400);
          playDirectDockTransition();
        }
      }, { passive: true });
...
console.log('Successfully written direct transition index.html! Bytes:', html.length);
