...
<body>
  ...
  <nav class="site-nav" aria-label="Main Navigation">
    <a href="#heroScrollFlow" class="nav-brand" onclick="window.__scrollToStory &amp;&amp; window.__scrollToStory(); return false;">OZAN ARDA</a>
  ...
  <!-- =========================================================================
       ...
       - Layer 3: 01 Hikayem (Untouched original 3D Meng-To Sketchbook iframe with 9 plates & loupe)
       ...
       ========================================================================= -->
  <div class="hero-scroll-flow" id="heroScrollFlow" style="height: 260vh;">
  ...
  <main class="main">
    ...
    <!-- [4] 04 İLETİŞİM & GÖREV TALEBİ -->
    <section class="section contact-section" id="contact" data-reveal>
      <div class="section-head">
        <div>
          <span class="section-folio"><span class="lang-tr">04 / KANALLAR &amp; İLETİŞİM</span><span class="lang-en">04 / DIRECT CHANNELS</span></span>
  ...
  <script>
    (function () {
      ...
      if ('requestIdleCallback' in window) {
        requestIdleCallback(() => setTimeout(hydrateStoryIframe, 200), { timeout: 1000 });
      } else {
        setTimeout(hydrateStoryIframe, 250);
      }

      // Pre-hydrate Shelf Iframe
      if ('requestIdleCallback' in window) {
        requestIdleCallback(() => setTimeout(hydrateShelfIframe, 600), { timeout: 2000 });
      } else {
      ...
      // - p in [0.00, 0.48]: 01 Hikayem (Original 3D Meng-To Sketchbook) active
      // - p in [0.48, 0.85]: 3D Multi-page fluttering riffle turns & hardcover clap shut into Slot 0
      // - p in [0.80, 1.00]: 02 Yeteneklerim (3D Shelf) active
      ...
      function onScrollHeroFlow() {
        ...
        // =========================================================================
        // 1. STAGE 1: 01 HİKAYEM (ORIGINAL 3D MENG-TO SKETCHBOOK) (p: 0.00 -> 0.54)
        // =========================================================================
        if (flowStoryLayer) {
          let storyOpacity = 1;
          if (p > 0.48 && p <= 0.54) {
        ...
        // =========================================================================
        // 3. STAGE 3: 02 YETENEKLERİM (3D SHELF IFRAME) (p: 0.80 -> 1.00)
        // =========================================================================
        if (flowShelfLayer) {
          let shelfFade = 0;
          if (p > 0.80) {
...
console.log('Successfully written pristine 60+ FPS transition index.html! Bytes:', html.length);
