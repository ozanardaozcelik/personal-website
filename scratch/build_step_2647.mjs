...
<body>
  ...
  <!-- =========================================================================
       HERO SCROLL FLOW (01 HOME FULLSCREEN -> 3D ZOOM OUT -> 02 MILESTONE TURNS -> 3D DOCK -> 03 SHELF)
       Exact full-screen original Hero Home, zooms out in 3D perspective to desk,
       turns 9 milestone pages as user scrolls, claps closed, and docks into 3D shelf!
       ========================================================================= -->
  <div class="hero-scroll-flow" id="heroScrollFlow" style="height: 520vh;">
    <div class="hero-sticky-viewport" id="heroStickyViewport">
      ...
      <!-- LAYER 2: 3D UNIFIED PHYSICAL SKETCHBOOK WITH MILESTONE PAGES 1-9 -->
      <div class="flow-layer flow-3d-book-layer" id="flow3dBookLayer" style="opacity: 0; visibility: hidden;">
        <div class="scroll-dock-book" id="scrollDockBook">
          ...
          <!-- SPREAD 1 LEFT: BASE WING (3 Yaş: İlk Bilgisayar) -->
          <div class="sdb-wing sdb-left" id="sdbWingLeft">
            <div class="sdb-face sdb-narrative-page" style="background: #ece7dc;">
      ...
      <!-- LAYER 3: 01 ANA SAYFA (ORIGINAL FULLSCREEN HERO SPREAD - ZOOMS OUT TO DESK) -->
  ...
  <script>
    (function () {
      ...
      // - Spread 0: Fullscreen Original Hero zooms out to desk in 3D perspective
      // - Spread 1 to 9: Sequential 3D Page Turns as scroll progresses
      ...
      function onScrollHeroFlow() {
        ...
        // =========================================================================
        // 1. STAGE 1: HERO HOME FULLSCREEN -> 3D ZOOM OUT ONTO DESK (p: 0.00 -> 0.12)
        // =========================================================================
        if (p <= 0.14) {
          const zoomP = smoothstep(p / 0.12);
          const scale = 1.0 - zoomP * 0.40; // 1.00 down to 0.60
          const rotateX = zoomP * 4.0;
          const translateY = -zoomP * 12;
          const shadowAlpha = zoomP * 0.24;
          const shadowY = zoomP * 28;
          const shadowBlur = zoomP * 56;
          ...
          // Fade out as it hands over to 3D book layer at p ~ 0.12
          let heroOpacity = 1;
          if (p > 0.09) {
            heroOpacity = Math.max(0, 1 - (p - 0.09) / 0.04);
          }
        ...
        // =========================================================================
        // 2. STAGE 2: 3D SKETCHBOOK FADES IN & 8 CHRONOLOGICAL PAGE TURNS (p: 0.10 -> 0.64)
        // =========================================================================
        if (flow3dBookLayer) {
          let bookOpacity = 0;
          if (p < 0.09) {
            ...
            bookOpacity = (p - 0.09) / 0.04;
          } else if (p <= 0.88) {
            bookOpacity = 1;
          } else if (p <= 0.94) {
            bookOpacity = Math.max(0, 1 - (p - 0.88) / 0.06);
          } else {
            bookOpacity = 0;
          }
          flow3dBookLayer.style.opacity = bookOpacity.toFixed(3);
          flow3dBookLayer.style.visibility = (p >= 0.09 && p <= 0.95) ? 'visible' : 'hidden';
        }

        if (p >= 0.09 && p <= 0.95 && scrollDockBook) {
          const turnStart = 0.14;
          const turnEnd = 0.64;
          const step = (turnEnd - turnStart) / 8; // ~0.0625 per page
          ...
          // =========================================================================
          // 3. STAGE 3: HARDCOVER WING CLAP SHUT (p: 0.64 -> 0.72)
          // =========================================================================
          let foldP = 0;
          if (p >= 0.72) {
            foldP = 1;
          } else if (p > 0.64) {
            foldP = smoothstep((p - 0.64) / 0.08);
          }
          ...
          // =========================================================================
          // 4. STAGE 4: CLOSED 3D LEATHER BOOK DOCKS INTO 3D SHELF (p: 0.72 -> 0.88)
          // =========================================================================
          if (p <= 0.72) {
            ...
            let dockP = smoothstep((p - 0.72) / 0.16);
        ...
        if (flowShelfLayer) {
          let shelfFade = 0;
          if (p > 0.72) {
            shelfFade = smoothstep((p - 0.72) / 0.16);
          }
          flowShelfLayer.style.opacity = shelfFade.toFixed(3);
          flowShelfLayer.style.visibility = p > 0.70 ? 'visible' : 'hidden';
...
console.log('Successfully written pristine 5-section index.html! Bytes:', html.length);
