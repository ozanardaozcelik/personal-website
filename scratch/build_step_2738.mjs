...
<body>
  ...
  <!-- =========================================================================
       01 ANA SAYFA: ORIGINAL FULLSCREEN HERO (PORTRAIT & WEBGL CYBORG REVEAL)
       ========================================================================= -->
  ...
  <!-- =========================================================================
       HERO SCROLL FLOW (02 HİKAYEM 3D SKETCHBOOK -> 3D RIFFLE DOCK -> 03 YETENEKLERİM 3D SHELF)
       380vh scroll track housing:
       - Layer 1 (Bottom): 03 Yeteneklerim (3D Shelf iframe)
       - Layer 2 (Middle): 3D Fluttering Leather Book Transition (Folds & Docks into Slot 0)
       - Layer 3 (Top): 02 Hikayem (Original Interactive 3D Sketchbook iframe with 9 Spreads)
       ========================================================================= -->
  <div class="hero-scroll-flow" id="heroScrollFlow" style="height: 380vh;">
    <div class="hero-sticky-viewport" id="heroStickyViewport">
      ...
      <!-- LAYER 2: 3D UNIFIED PHYSICAL SKETCHBOOK RIFFLE FOLD & DOCK TRANSITION -->
      <div class="flow-layer flow-3d-book-layer" id="flow3dBookLayer">
        <div class="scroll-dock-book" id="scrollDockBook">
          ...
          <!-- HARDCOVER RIGHT WING (CLAPS SHUT INTO LEATHER BROWN COVER) -->
  ...
  <script>
    (function () {
      ...
      // Hydrate Story Iframe after initial render
      if ('requestIdleCallback' in window) {
        requestIdleCallback(() => setTimeout(hydrateStoryIframe, 200), { timeout: 1500 });
      } else {
        setTimeout(hydrateStoryIframe, 300);
      }

      // Hydrate Shelf Iframe on demand or after idle delay
      if ('requestIdleCallback' in window) {
      ...
      // 2. Pure Scroll-Driven Continuous Timeline for HeroScrollFlow:
      // - p in [0.00, 0.20]: 02 Hikayem (Interactive 3D Sketchbook) active
      // - p in [0.20, 0.60]: 3D Multi-page fluttering riffle turns & hardcover clap shut
      // - p in [0.60, 0.85]: Closed 3D leather volume scales and docks into 3D shelf slot 0
      ...
      // Shelf active state management (WebGL resource preservation)
      ...
      function onScrollHeroFlow() {
        ...
        if (flowContainer) {
          ...
          const inShelfRange = (scrollY >= flowTop + (flowH - winH) * 0.60) && (scrollY <= flowTop + flowH + 300);
          setShelfActiveState(inShelfRange);
        }

        if (flowContainer && flowStoryLayer && flow3dBookLayer && flowShelfLayer && scrollDockBook) {
          ...
          if (totalTravel > 0) {
            ...
            if (Math.abs(p - lastP) >= 0.0004 || Math.abs(scrollY - lastScrollY) >= 1) {
              lastP = p;
              lastScrollY = scrollY;

              // STAGE 1: 02 Hikayem (Sketchbook Iframe)
              // Visible & interactive at top of flow
              let storyOpacity = 1;
              if (p > 0.20) {
                storyOpacity = Math.max(0, 1 - (p - 0.20) / 0.08);
              }
              flowStoryLayer.style.opacity = storyOpacity.toFixed(3);
              flowStoryLayer.style.visibility = storyOpacity > 0 ? 'visible' : 'hidden';
              flowStoryLayer.style.pointerEvents = (p <= 0.20 && rect.top <= 50) ? 'auto' : 'none';

              // STAGE 2: 3D Fluttering Book Transition Layer
              let book3dOpacity = 0;
              if (p < 0.18) {
                book3dOpacity = 0;
              } else if (p < 0.26) {
                book3dOpacity = (p - 0.18) / 0.08;
              } else if (p <= 0.82) {
                book3dOpacity = 1;
              } else if (p <= 0.90) {
                book3dOpacity = Math.max(0, 1 - (p - 0.82) / 0.08);
              } else {
                book3dOpacity = 0;
              }
              flow3dBookLayer.style.opacity = book3dOpacity.toFixed(3);
              flow3dBookLayer.style.visibility = (p >= 0.18 && p <= 0.92) ? 'visible' : 'hidden';

              // 3D Leaf Flutter Riffle Sequence (Fast, realistic fluttering pages)
              if (p >= 0.18 && p <= 0.92) {
                // Leaf 1
                let leaf1P = 0;
                if (p >= 0.38) leaf1P = 1;
                else if (p > 0.26) leaf1P = smoothstep((p - 0.26) / 0.12);
                ...
                let leaf2P = 0;
                if (p >= 0.46) leaf2P = 1;
                else if (p > 0.34) leaf2P = smoothstep((p - 0.34) / 0.12);
                ...
                let leaf3P = 0;
                if (p >= 0.54) leaf3P = 1;
                else if (p > 0.42) leaf3P = smoothstep((p - 0.42) / 0.12);
                ...
                let foldP = 0;
                if (p >= 0.62) foldP = 1;
                else if (p > 0.50) foldP = smoothstep((p - 0.50) / 0.12);
                ...
                if (p <= 0.62) {
                  ...
                  let dockP = smoothstep((p - 0.62) / 0.18);
              ...
              // STAGE 3: 03 Yeteneklerim (3D Shelf Iframe)
              let shelfFade = 0;
              if (p > 0.70) {
        ...
        // =========================================================================
        const flowTop = getDocTop(flowContainer);
        const flowH = flowContainer ? flowContainer.offsetHeight : 0;
        ...
        } else if (flowContainer && scrollY >= flowTop + (flowH - winH) * 0.65) {
          if (btnNavSkills) btnNavSkills.classList.add('is-active');
        } else if (flowContainer && scrollY >= flowTop - winH * 0.3) {
      ...
      function scrollToStory() {
        if (flowContainer) {
          const flowTop = getDocTop(flowContainer);
          window.scrollTo({ top: flowTop, behavior: 'smooth' });
        }
      }
      function scrollToShelf() {
        if (flowContainer) {
          ...
          window.scrollTo({ top: flowTop + totalTravel * 0.95, behavior: 'smooth' });
        }
      }
