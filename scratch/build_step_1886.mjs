...
<body>
  ...
  <!-- =========================================================================
       01 ANA SAYFA: INTERACTIVE FLUID PORTRAIT & ROBOT REVEAL HERO (WARM LIGHT PAPER THEME)
       ========================================================================= -->
  ...
  <!-- =========================================================================
       ...
       Warm light paper background throughout the entire flow!
       ========================================================================= -->
  <div class="hero-scroll-flow" id="heroScrollFlow">
    <div class="hero-sticky-viewport" id="heroStickyViewport">
      ...
      <!-- LAYER 2: 3D DOCKING BOOK (Smooth 3D folding & gliding on warm paper background) -->
    ...
    <div class="footer-middle-grid">
      <div>
        ...
        <ul class="footer-links">
          <li><a href="#heroHome" onclick="window.__scrollToHome &amp;&amp; window.__scrollToHome(); return false;"><span class="lang-tr">01 Ana Sayfa</span><span class="lang-en">01 Home</span></a></li>
          <li><a href="#heroScrollFlow" onclick="window.__scrollToStory &amp;&amp; window.__scrollToStory(); return false;"><span class="lang-tr">02 Hikayem (Eskiz Defteri)</span><span class="lang-en">02 My Story (Sketchbook)</span></a></li>
          <li><a href="#heroScrollFlow" onclick="window.__scrollToShelf &amp;&amp; window.__scrollToShelf(); return false;"><span class="lang-tr">03 Yeteneklerim (Kitaplık)</span><span class="lang-en">03 Capabilities (Shelf)</span></a></li>
          <li><a href="#projects-section" onclick="window.__scrollToProjects &amp;&amp; window.__scrollToProjects(); return false;"><span class="lang-tr">04 Projelerim (GitHub)</span><span class="lang-en">04 Projects (GitHub)</span></a></li>
          <li><a href="#contact" onclick="window.__scrollToContact &amp;&amp; window.__scrollToContact(); return false;"><span class="lang-tr">05 İletişim</span><span class="lang-en">05 Contact</span></a></li>
        </ul>
      </div>
      <div>
      ...
      <div>
        ...
        <ul class="footer-links">
          <li><span>VOL. 2026 // BUILD 5.2</span></li>
  ...
  <script>
    (function () {
      ...
      // Scroll Handler with Comfortable Reading Buffer before folding
      function onScrollHeroFlow() {
        ...
        // Fully active and solid during reading zone (p: 0.00 -> 0.18)
        // Crossfades out as folding begins (p: 0.18 -> 0.26)
        if (flowStoryLayer) {
          let storyOpacity = 1;
          if (p > 0.18) {
            storyOpacity = Math.max(0, 1 - (p - 0.18) / 0.08);
          }
          flowStoryLayer.style.opacity = storyOpacity.toFixed(3);
          flowStoryLayer.style.pointerEvents = p <= 0.20 ? 'auto' : 'none';
        }

        // 2. 3D Closing book layer:
        // Crossfades in at p=0.18, solid during folding (p: 0.24 -> 0.85), fades out as shelf docks (p: 0.85 -> 0.94)
        if (flow3dBookLayer) {
          let bookOpacity = 0;
          if (p < 0.18) {
            bookOpacity = 0;
          } else if (p < 0.24) {
            bookOpacity = (p - 0.18) / 0.06;
          } else if (p <= 0.85) {
            bookOpacity = 1;
          } else if (p <= 0.94) {
        ...
        // 3. Right wing cover folding shut (p: 0.20 -> 0.55):
        let foldP = 0;
        if (p > 0.20) {
          foldP = smoothstep((p - 0.20) / 0.35);
        }
        const rightDeg = -foldP * 180;
        sdbWingRight.style.transform = 'rotateY(' + rightDeg.toFixed(2) + 'deg)';

        // 4. Zoom out in 3D perspective and dock into shelf slot (p: 0.50 -> 0.88):
        let zoomP = 0;
        if (p > 0.50) {
          zoomP = smoothstep((p - 0.50) / 0.38);
        }
        ...
        // 5. Background shelf fades in (p: 0.45 -> 0.85):
        if (flowShelfLayer) {
          let shelfFade = 0;
          if (p > 0.45) {
            shelfFade = smoothstep((p - 0.45) / 0.40);
          }
          flowShelfLayer.style.opacity = shelfFade.toFixed(3);
          flowShelfLayer.style.pointerEvents = p >= 0.85 ? 'auto' : 'none';
        }

        // 6. 5-Section Nav Tracking (Using exact getDocTop)
        const scrollY = window.scrollY;
        const flowTop = getDocTop(flowContainer);
        const flowH = flowContainer ? flowContainer.offsetHeight : 2000;
        const shelfThreshold = flowTop + (flowH - winH) * 0.5;
      ...
      function scrollToShelf() {
        if (flowContainer) {
          const top = getDocTop(flowContainer);
          const targetY = top + flowContainer.offsetHeight - window.innerHeight;
